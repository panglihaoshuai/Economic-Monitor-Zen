...
1. 更新静态日期索引：
   -- 删除旧的5年索引
   DROP INDEX IF EXISTS economic_data_recent_5yr_idx;
   -- 创建新的5年索引（使用当前年份）
   CREATE INDEX economic_data_recent_5yr_idx ON economic_data (series_id, date DESC)
   WHERE date >= '2021-01-01'::date; -- 更新为5年前的日期

2. 或者使用函数（需要 IMMUTABLE 函数）：
   -- 创建自定义 IMMUTABLE 函数
   CREATE OR REPLACE FUNCTION static_cutoff_date() 
   RETURNS date AS $$
   SELECT '2020-01-01'::date;  -- 手动更新这个日期
   $$ LANGUAGE sql IMMUTABLE;
   
   -- 然后在索引中使用
   CREATE INDEX economic_data_dynamic_idx ON economic_data (series_id, date DESC)
   WHERE date >= static_cutoff_date();
*/
